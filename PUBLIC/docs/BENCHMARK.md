...
2. **DeepSeek and GLM do NOT work**: They return empty content with the VBP format
...
Backup:    GPT-5.4-mini  → 8s,  subscription, 100% ← always available
```

## Test methodology

- **Image**: Full-page screenshot of elmundo.es (1280×800, Chrome headless)
- **VBP**: Generated by `src/samson_vision.py` with Tesseract OCR (Spanish + English)
- **Models**: Tested via OpenCode Go API, MiniMax API, and Codex CLI
- **Prompt**: Same system prompt for all models (interpret VBP as if seeing the image)
